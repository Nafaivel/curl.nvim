...
```lua
{
  ...
  -- Include stderr before stdout in the output buffer for successful requests
  show_stderr = false,
...
- scoped/plugin-managed curl buffers: the working directory from when the buffer was opened
- manually opened `.curl` files: that file's directory

If a `---source` file is missing or unreadable, execute/export aborts with an error.
